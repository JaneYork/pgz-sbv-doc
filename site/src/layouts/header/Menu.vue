<template>
  ...
  <template v-else>
    ...
    >
      ...
      <a-select-option value="v1.x" @click="changeVersion('v1')">
        v1.x (Safari不适配)
      </a-select-option>
      <a-select-option v-if="false" value="1.x" @click="changeVersion('1x')">
        1.x (For Vue 2)
      </a-select-option>
    </a-select>
    <a-button
...
export default defineComponent({
  ...
  setup() {
    ...
    const changeVersion = v => {
      // location.href = `https://${v}.doc.shidongvr.com${route.fullPath}`;
      location.href = `https://${v}.doc.shidongvr.com/`;
    };
    return {
      onLangChange,
      antdVersion,
